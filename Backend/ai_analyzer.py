...
def analyze_all_parameters(soil_params, nutrients, game_metrics):
    ...
    parametros["drenagem"] = analyze_parameter("drenagem", soil_params.get("drenagem", 75))
    parametros["aeracao"] = analyze_parameter("aeracao", soil_params.get("aeracao", 20))
    parametros["compactacao"] = analyze_parameter("compactacao", soil_params.get("compactacao", 1.3))
    parametros["atividadeMicrobiana"] = analyze_parameter("atividadeMicrobiana", soil_params.get("atividadeMicrobiana", 75))
...
def determine_status(param_name, value, ranges):
    ...
    if param_name in ["salinidade", "condutividade", "compactacao"]:
    ...
    elif param_name == "atividadeMicrobiana":
        # Parâmetro que tem só mínimo
        min_val = ranges["min"]
        if value >= min_val:
            return STATUS_IDEAL
...
def get_suggestion(param_name, value, status):
    ...
    suggestions = {
        ...
        "drenagem": {
            STATUS_IDEAL: "Manter sistema de drenagem atual",
            STATUS_ATENCAO: "Melhorar drenagem com sulcos",
            STATUS_CRITICO: "Instalar sistema de drenagem urgente"
        },
        "aeracao": {
            STATUS_IDEAL: "Manter práticas de cultivo atuais",
            STATUS_ATENCAO: "Fazer subsolagem leve",
            STATUS_CRITICO: "Descompactar solo urgentemente"
        },
        "compactacao": {
            STATUS_IDEAL: "Solo com estrutura adequada",
            STATUS_ATENCAO: "Evitar tráfego em solo úmido",
            STATUS_CRITICO: "Subsolagem profunda necessária"
        },
        "atividadeMicrobiana": {
            STATUS_IDEAL: "Manter adição de matéria orgânica",
            STATUS_ATENCAO: "Aumentar matéria orgânica",
            STATUS_CRITICO: "Inoculação microbiana urgente"
        }
    }
...
def calculate_soil_health(soil_data: Optional[Dict]) -> float:
    ...
    # Novos parâmetros físicos (40 pontos total)
    drenagem = soil_params.get("drenagem", 75)
    if 60 <= drenagem <= 90:
        score += 10
    elif 50 <= drenagem <= 95:
        score += 5
    
    aeracao = soil_params.get("aeracao", 20)
    if 10 <= aeracao <= 30:
        score += 10
    elif 5 <= aeracao <= 35:
        score += 5
    
    compactacao = soil_params.get("compactacao", 1.3)
    if compactacao <= 2.0:
        score += 10
    elif compactacao <= 2.5:
        score += 5
    
    atividade = soil_params.get("atividadeMicrobiana", 75)
    if atividade >= 50:
        score += 10
    elif atividade >= 30:
        score += 5
    
    # Nutrientes (20 pontos)
