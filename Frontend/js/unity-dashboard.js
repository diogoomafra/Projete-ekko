...
const UnityDashboard = {
    ...
    async loadUnityProfile(unityId) {
        ...
        try {
            ...
            // Carregar monitoramento por último para garantir que o Unity ID esteja disponível
            setTimeout(() => {
            ...
            <div class="unity-card" style="margin-bottom: 2rem;">
                ...
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1.5rem;">
                    <div style="background: linear-gradient(135deg, rgba(34, 197, 94, 0.1), rgba(34, 197, 94, 0.05)); padding: 1.5rem; border-radius: 12px; border-left: 4px solid var(--primary-green);">
                        ...
                        <p style="color: var(--gray-700); margin: 0; line-height: 1.5;">Análise inteligente de 12 parâmetros do solo + Chatbot treinado com fontes renomadas do setor agrícola brasileiro</p>
                    </div>
                ...
                <div class="unity-info-grid" style="grid-template-columns: 1fr 1fr;">
                    <div style="background: rgba(255,255,255,0.8); padding: 1.5rem; border-radius: 16px; border: 1px solid var(--gray-200);">
                        ...
                        <select id="unity-heatmap-parameter" onchange="UnityDashboard.updateHeatmapView()" style="width: 100%; padding: 0.75rem 1rem; border: 2px solid var(--gray-200); border-radius: 12px; font-size: 1rem; background: white;">
                            ...
                            <option value="condutividade">Condutividade (dS/m)</option>
                            <option value="drenagem">Drenagem (%)</option>
                            <option value="aeracao">Aeração (%)</option>
                            <option value="compactacao">Compactação (g/cm³)</option>
                            <option value="atividadeMicrobiana">Atividade Microbiana</option>
                            <option value="nitrogenio">Nitrogênio (N)</option>
                            <option value="fosforo">Fósforo (P)</option>
                            <option value="potassio">Potássio (K)</option>
    ...
    getParametroIcon(param) {
        const icons = {
            ...
            'drenagem': 'filter',
            'aeracao': 'wind',
            'compactacao': 'compress-arrows-alt',
            'atividadeMicrobiana': 'bacteria'
        };
    ...
    getParametroNome(param) {
        const nomes = {
            ...
            'atividadeMicrobiana': 'Atividade Microbiana'
        };
    ...
    showSection(sectionId, clickedElement = null) {
        ...
        const contextualInfo = {
            ...
            'ai': 'Análise baseada em 12 parâmetros de solo • IA treinada com fontes brasileiras',
            ...
            'monitoring': 'Acompanhe em tempo real os dados coletados por tecnologia IoT simulada',
        ...
        dadosFiltrados.forEach(item => {
            ...
            switch(parametro) {
                ...
                case 'condutividade':
                    ...
                    break;
                case 'drenagem':
                    valor = Number(soilParams.drenagem || 0);
                    break;
                case 'aeracao':
                    valor = Number(soilParams.aeracao || 0);
                    break;
                case 'compactacao':
                    valor = Number(soilParams.compactacao || 0);
                    break;
                case 'atividadeMicrobiana':
                    valor = Number(soilParams.atividadeMicrobiana || 0);
                    break;
                case 'nitrogenio':
                    const nutrients1 = item.nutrients || {};
                    valor = Number(nutrients1.nitrogenio || 0);
                    break;
                case 'fosforo':
                    const nutrients2 = item.nutrients || {};
                    valor = Number(nutrients2.fosforo || 0);
                    break;
                case 'potassio':
                    const nutrients3 = item.nutrients || {};
                    valor = Number(nutrients3.potassio || 0);
                    break;
                case 'npk':
                ...
                <div class="unity-card" style="border-left: 4px solid var(--secondary-green);">
                    ...
                    <div>
                        ...
                        <div class="unity-data-row">
                            ...
                            <span><strong>CORS:</strong> Middleware para integração</span>
                        </div>
                    </div>
                </div>
    ...
    loadMonitoringContent(data) {
        ...
        if (typeof MonitoringModule !== 'undefined') {
            ...
            // Garantir que o Unity ID está disponível globalmente
            window.currentUnityId = this.currentUnityId;
