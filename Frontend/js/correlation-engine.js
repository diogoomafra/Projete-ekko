...
const CorrelationEngine = {
    ...
    analyzeCorrelations(data) {
        console.log('Analisando correlações com', data.length, 'registros');
        ...
        for (let i = 0; i < parameters.length; i++) {
            for (let j = i + 1; j < parameters.length; j++) {
                ...
                const values1 = data.map(d => parseFloat(d[param1])).filter(v => !isNaN(v) && v !== undefined);
                const values2 = data.map(d => parseFloat(d[param2])).filter(v => !isNaN(v) && v !== undefined);
                
                console.log(`${param1} vs ${param2}: ${values1.length} valores válidos`);
                
                if (values1.length >= 2 && values2.length >= 2 && values1.length === values2.length) {
                    const correlation = this.calculateCorrelation(values1, values2);
                    console.log(`Correlação ${param1}-${param2}:`, correlation);
                    
                    if (Math.abs(correlation) > 0.2) { // Reduzir threshold para ver mais correlações
                        correlations.push({
                            param1,
                            param2,
    ...
    getCorrelationStrength(r) {
        ...
        if (abs >= 0.7) return 'muito_forte';
        if (abs >= 0.5) return 'forte';
